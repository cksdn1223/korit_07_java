...
public class Main {
    public static void main(String[] args) {
        ...
        Animal animal1 = dog1;  //재정의된 method 가 호출됨
        animal1.makeSound();
        ...
        /*
            ...
            1. Override 가 적용된 메서드, 즉 자식 클래스의 메서드가 호출됨.
            2. 클래스B(자식 클래스)의 고유 메서드는 호출 불가능.

            그럼 어차피 재정의된 method 버전으로 호출이 될거라면 애초에
            클래스B 객체명 = new 클래스B(); 로 생성해버리면 재정의된
            method도 호출되고 고유 method도 호출할 수 있으니까 골치
            안아픈거 아닌가??
        ...
        // 업캐스팅이 이루어지지 않고 그냥 Dog dog1 = new Dog(); 로 만든 객체 역시 true
        boolean result3 = dog1 instanceof Animal;   //true
        boolean result4 = animal2 instanceof Dog;   // true
        // animal3의 경우 Animal animal3 = new Animal(); 로 생성했음
        boolean result5 = animal3 instanceof Animal;    // true
        ...
        다운캐스팅은 명시적으로 이루어져야함
        방법
        (클래스명)객체명; -> (int)2.3; 과 같은 방식. conversion의 객체 버전
        그래서 기본 자료형의 형변환에서 벗어나서 참조 자료형가지 고려하셔야 합니다.
         */

        Dog dog2 = (Dog) animal2; // 다운캐스팅 방법 -> Animal animal2 = new Dog(); 으로 생성해서 dog2 객체명은 없습니다.
        dog2.makeSound();
        dog2.fetch();
        // dog2로 새로운 객체명을 선언하고 거기에 명시적 참조 자료형 캐스팅을
        // 적용하여 Animal 클래스의 인스턴스인 animal2를 down캐스팅 했습니다.
        // 그 결과 원래도 되던 재정의 버전의 makeSound() 호출은 변함이 없고,
        // Dog클래스의 고유 메서드인 fetch()도 호출이 가능합니다.
...
//        dog3.makeSound(); // 그러니까 얘는 지금 호출도 못해보는 상황
        /*
            ...
            (A a = new A();)이기 때문에
            하위 클래스의 인스턴스로 다운캐스팅 하는것이 불가능합니다.
        ...
        System.out.println("animal3은 다운캐스팅 안되니까 검증 후에 다시 시도");
