...
public class StrMap {
    ...
    public static void main(String[] args) {
        ...
        /*
        List 의 경우 순서가 있기 때문에 기본적으로 index를 통한 조회가 가능합니다.
        하지만 Map의 경우 아까 출력겨로가에서 본 것처럼 순서를 보장하지 않기 때문에
        .get("키값"); 을 통해 조회할 수 있습니다.
         */
        System.out.println(strMap1.get("kor2025005") + " 님");
        ...
        // 특정 key의 value 수정
        // # 1. 아까 말한 것처럼 key에는 value가 하나 밖에 없기 대문에 최근 걸로 덮어쓰기 됩니다.
        ...
        System.out.println("kor2025005 학생 존재 여부 : " + searchKeyFlag1);
        ...
        // 이상의 코드는 strMap1의 key-value 쌍을 Set로 바꾸는 메서ㅡ인 .entrySet()의 호출 결과를
...
//        List<Map.Entry<String, String>> entryList2 = strMap1.entrySet();
        ...
        System.out.println(entryList1); // list로 바뀌었으니까 element 추출이 가능해진다.
        // 이상까지의 과정에서 Map -> Set -> List로 바꾸는 과정을 코드로 작성했는데
        // Map -> List 로 바로 가는 게 가능한지 여부를 따질 필요가 있습니다.

        // .sort()도 가능하고 추출도 가능하다
...
//        Set<String> valueSet = strMap1.values();
        /*
        key는 중복을 허용하지 않기때문에 Set으로 바꾸는게 가능한 반면,
        복수의 key에서 동일한 value가 존재할 수 있기 때문에 직접적으로
        Set으로 바꾸지 않습니다.
         */
        Collection<String> values = strMap1.values();
        System.out.println(values);
        // Collection 자료형은 처음 써봤음.
